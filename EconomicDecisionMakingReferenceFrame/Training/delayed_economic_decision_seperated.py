...
class DelayedEconomicDecision_SeparatedSpatialTask(Task):
    """Delayed economic decision task with factorized value and location inputs.

    Offer identity/quantity and left/right location are presented through
    separate channels during the same offer epoch. At target time, the network
    reports the side of the chosen offer.
    """
    ...
    def __init__(self, dt, tau, T, N_batch=None,
        ...
        super(DelayedEconomicDecision_SeparatedSpatialTask, self).__init__(
            len(self._input_names), 2, dt, tau, T, N_batch)
    ...
    def _quantity_channel(self, juice):
        return self._input_index['q%s_norm' % juice]

    def _location_channel(self, side):
        return self._input_index['offer_%s' % side]
    ...
    def trial_function(self, t, params):
        ...
        if stimulus_1_onset <= t < stimulus_1_offset:
            x_t[self._quantity_channel(juice1)] += q1
            x_t[self._location_channel(side1)] += 1

        if stimulus_2_onset <= t < stimulus_2_offset:
            x_t[self._quantity_channel(juice2)] += q2
            x_t[self._location_channel(side2)] += 1
