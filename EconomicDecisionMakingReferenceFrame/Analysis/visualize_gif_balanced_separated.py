...
modelVariant = 'balanced_separated'
outputMode = 'spatial'
...
dataRoot = os.path.join(
    ...
    'Training',
    'savedForLocal',
    'spatialTask_balanced_separated',
)
outputRoot = os.path.join(analysisDir, 'new_gif', 'gif_balanced_separated')
...
def main():
    ...
    for name in sorted(os.listdir(dataRoot)):
        ...
        if os.path.isdir(dataDir):
            outputDir = os.path.join(outputRoot, name)
            generateGif(dataDir,outputMode,outputDir)
...
def generateSnapShot_Encoding(dirPath,activityFilename,outputMode,gif_name,figsize,outputDir):
    image_files=[]
    frames = []
    ...
    for i in range(num_frames):
        ...
        # Save the figure
        filename = os.path.join(outputDir,'temp',f"{gif_name}_Encoding_frame_{i:03d}.png")
...
def generateSnapShot_Choice(dirPath,activityFilename,outputMode,gif_name,figsize,outputDir):
    image_files=[]
    frames = []
    ...
    for i in range(num_frames):
        ...
        # Save the figure
        filename = os.path.join(outputDir,'temp',f"{gif_name}_frame_{i:03d}.png")
        plt.savefig(filename, bbox_inches='tight')
        # plt.close(fig)
...
def generateSnapShot_Action(dirPath,activityFilename,outputMode,gif_name,figsize,outputDir):
    image_files=[]
    frames = []
...
def generateGif(dirPath,outputMode,outputDir):
    ...
    gif_path_encoding = os.path.join(outputDir,gif_name_encoding+'.gif')
    image_files_encode,frames_encode = generateSnapShot_Encoding(dirPath,activityFileName,outputMode,gif_name_encoding,figsize,outputDir)
    generateGif_From((image_files_encode,frames_encode),gif_path_encoding,3)
    gif_name_choice = 'gifChoice'
    gif_path_choice = os.path.join(outputDir,gif_name_choice+'.gif')
    image_files_choice,frames_choice = generateSnapShot_Choice(dirPath,activityFileName,outputMode,gif_name_choice,figsize,outputDir)
    generateGif_From((image_files_choice,frames_choice),gif_path_choice,3)
    gif_name_action = 'gifAction'
    gif_path_action = os.path.join(outputDir,gif_name_action+'.gif')
    image_files_action,frames_action = generateSnapShot_Action(dirPath,activityFileName,outputMode,gif_name_action,figsize,outputDir)
    generateGif_From((image_files_action,frames_action),gif_path_action,3)

    gif_path_full = os.path.join(outputDir,'gifFull.gif')
